...
from tool.models import TaskWithResources, TaskWithSolutionStructure, State, ResourceForm, Resource
...
def collect_resources(draft: TaskWithSolutionStructure) -> TaskWithResources:
    query = (
        f"Task: {draft.task}\n"
    ...
    for request in requested_sources:
        ...
        result = _resource_provider.invoke({"messages": messages})["messages"][-1].content
        requested_sources[request] = str(result)

    return TaskWithResources(
        task=draft.task,
        context=draft.context,
