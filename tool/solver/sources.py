...
_tools = [WikipediaQueryRun(api_wrapper=WikipediaAPIWrapper())]
_resource_provider = create_react_agent(_model, tools=_tools)


def collect_sources(draft: TaskWithSolutionDraft) -> TaskWithSourceAugmentedDraft:
    query = (
        f"Task: {draft.task}\n"
        f"Context: {draft.context}\n"
        f"Solution draft: {draft.solution_draft}"
    )
    ...
    return TaskWithSourceAugmentedDraft(
        ...
        tests=draft.tests,
        solution_draft=draft.solution_draft,
        sources=requested_sources,
    )


def print_sources(draft_with_sources: TaskWithSourceAugmentedDraft) -> State:
