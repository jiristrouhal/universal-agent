    ...
    for path in paths:
        if os.path.isfile(path):
            ...
            if file_name.endswith(".py"):
                pattern, dir_ = file_name, os.path.dirname(path)
        else:
            pattern, dir_ = "test_*.py", path
        suite.addTests(unittest.TestLoader().discover(dir_, pattern=pattern))
